...
- It is free and open software, which means that it is 
flexible enough to create packages for specific computational purposes

- R functions and packages are extremely well documented and information
...
3. Choose a download website from the list
4. Choose your operating system (Windows, MAC, Linux)
5. Click **base**
6. Click on **Download R.3.2.3 for** your operating system (OS)
...
A new *script* window should open. You can also do this by simply typing
**CTRL + Shift + N**
...
To do so you must type the operation in your script window and then press **CTRL + ENTER**.
Make sure the crusor is on the command line. 
...
Notice that the presence of parentheses *()* indicates which operations should be carried out
first.
...
The working directory is the folder in which you are currently working in. 
This means that R will:
...
You can also check what is currently within the directory by using ```dir()```.

### Exercise 4

1. Create a folder called *R_wkshp* under **Documents**
2. Find your current working directory 
3. Set your working directory to the *R_wkshp* folder you just created
...
R can also carry out a variety of statistical analyses that are available in *packages* or *libraries*. Some of these are downloaded upon R installation, however some of the more
specific computations require that you download additional *packages*. 
...
**Installing Packages:** If we wish to install a package, we can click on the **Install** button on the **Package* tab and type in the 
name(s) or the package(s) we need. 
