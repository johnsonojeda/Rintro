...
nrow(cgd)    # Number of rows in mtcars

ncol(cgd) # Number of columns in mtcars
```
...
cgd = head(cgd)  # New data frame with only first 6 rows
...
There may be times when we might want to manipulate smaller sets of the original dataframe. For example we may be interested only in those entries where patients are under 20 years of age.
...
head(Placebo)                                  # Print first values of the dataframe
```
